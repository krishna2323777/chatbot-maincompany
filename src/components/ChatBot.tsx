...
export const ChatBot: React.FC = () => {
  ...
  if (isMinimized) {
    return <div className="fixed bottom-4 right-4 z-50">
        <button onClick={toggleChat} className="bg-gradient-to-r from-pink-500 to-purple-600 text-white rounded-full w-14 h-14 flex items-center justify-center shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-105 active:scale-95" aria-label="Open chat">
          <MessageCircleIcon className="h-6 w-6" />
        </button>
      </div>;
  }
  return <div className={`fixed inset-0 md:inset-auto md:bottom-6 md:right-6 z-50 transition-all duration-300 ${isOpen ? 'opacity-100 scale-100' : 'opacity-0 scale-95 pointer-events-none'}`}>
      <div className="flex flex-col w-full h-full md:w-[400px] md:h-[600px] md:rounded-xl overflow-hidden shadow-xl bg-white">
        <div className="bg-gradient-to-r from-pink-500 to-purple-600 text-white p-3 flex items-center justify-between">
          ...
          <div className="flex space-x-2">
            <button className="text-white/80 hover:text-white p-1 transition-colors duration-200" onClick={toggleChat} aria-label="Minimize chat">
              <MinimizeIcon className="h-4 w-4" />
            </button>
            <button className="text-white/80 hover:text-white p-1 transition-colors duration-200" onClick={closeChat} aria-label="Close chat">
